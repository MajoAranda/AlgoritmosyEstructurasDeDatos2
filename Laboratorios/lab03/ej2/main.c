...
int main(int argc, char *argv[]) {
    ...
    //Minimum temps
    int min=temp_min(array);
    printf("The minimum tempetarute in Córdoba between 1980-2016 is:%d\n",min);

    //Max temps
    int max[YEARS];
    max_temp(array,max);
    printf("Maximum temperature per month in Córdoba:\n");
    for(unsigned int i=0;i<=YEARS;i++){
        printf("%d,",max[i]);
    }
    printf("\n");
    
    //Months with max rainfalls
    month_t months[YEARS];
    max_rainfall(array,months);
    printf("Months with the most amount of rainfalls per year:\n");
    for(unsigned int i=0;i<=YEARS;i++){
        printf("%d,",months[i]);
    }
    printf("\n");
